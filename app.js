...
app.get('/view_transcripts/:room_id', chat.singleTranscript);
...
io.sockets.on('connection', function(socket) {
	...
	socket.on('set username', function(userName, role) {
		if (clients[userName] === undefined) {
			...
			// the following is only for role = 'operator'
			if (role != 'requester') {
				lobbyTransactionsForOperator(userName, socket);
			}
	...
	socket.on('set operator', function(userName) {
		socket.operator = userName;
	})
	...
	socket.on('notify', function(rId, userName, notification) {
		var newClientsList = getClientsListInRoom(rId);
		socket.broadcast.to(rId).emit('sendNotification', userName, notification, newClientsList);
	});
	...
	socket.on('joinRoom', function(joiner, rId) {
		...
		var clientsList = getClientsListInRoom(rooms[rId]);
		socket.emit('switchRoom', rooms[rId], clientsList);

		var oldClientsList = getClientsListInRoom(socket.room);
		socket.broadcast.to(socket.room).emit('switchRoomBroadcast', joiner, 'left', oldClientsList);

		socket.room = rooms[rId];
		var newClientsList = getClientsListInRoom(socket.room);
	...
	getCurrentRooms(socket);
	requesterExit(socket);
})

function lobbyTransactionsForOperator(userName, socket) {
...
function userJoined(uName) {
	var clientsList = getClientsListInRoom('lobby');
...
function userNameAvailable(sId, uName) {
	...
	// sending to that specific socket
...
function getCurrentRooms(socket) {
...
function requesterExit(socket) {
	socket.on('exit', function(curRoom) {
		...
		red_cli.get(sms_pool_id, function (err, reply) {
			...
			chatTranscript.operator = socket.operator;
